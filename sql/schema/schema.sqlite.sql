...
(
    ...
    password CHAR NOT NULL DEFAULT '123456',
...
(
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    id_device INTEGER CONSTRAINT fk_device_id REFERENCES device(id),
    user_apply INTEGER CONSTRAINT fk_users_id REFERENCES users(id),
    user_release INTEGER CONSTRAINT fk_users_id REFERENCES users(id),
    time_apply CHAR,
    time_release CHAR
);
...
(
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    id_device INTEGER CONSTRAINT fk_device_id REFERENCES device(id),
    ip_addr char UNIQUE NOT NULL,
    account CHAR NOT NULL DEFAULT 'root',
    password CHAR NOT NULL DEFAULT 'root'
);
...
(
    ...
    meta_update_time CHAR,
    meta_update_user CHAR,
    id_apply_history INTEGER NOT NULL DEFAULT 0
);
...
-- Table: apply_history
--
CREATE TRIGGER fki_apply_history_device_id
BEFORE INSERT ON apply_history
FOR EACH ROW BEGIN
    SELECT RAISE(ROLLBACK, 'insert on table "apply_history" violates foreign key constraint "fki_apply_history_device_id"')
    WHERE (SELECT id FROM device WHERE id = NEW.id_device) IS NULL;
END;

CREATE TRIGGER fki_apply_history_users_id
BEFORE INSERT ON apply_history
FOR EACH ROW BEGIN
    SELECT RAISE(ROLLBACK, 'insert on table "apply_history" violates foreign key constraint "fki_apply_history_users_id"')
    WHERE (SELECT id FROM users WHERE id = NEW.user_apply) IS NULL;
END;

CREATE TRIGGER fku_apply_history_users_id
BEFORE UPDATE ON apply_history
FOR EACH ROW BEGIN
    SELECT RAISE(ROLLBACK, 'update on table "apply_history" violates foreign key constraint "fku_apply_history_users_id"')
    WHERE (SELECT id FROM users WHERE id = NEW.user_release) IS NULL;
END;

--
-- Table: device_ip
--
CREATE TRIGGER fki_device_ip_id
BEFORE INSERT ON device_ip
FOR EACH ROW BEGIN
    SELECT RAISE(ROLLBACK, 'insert on table "device_ip" violates foreign key constraint "fki_device_ip_id"')
    WHERE (SELECT id FROM device WHERE id = NEW.id_device) IS NULL;
END;

CREATE TRIGGER fku_device_ip_id
BEFORE UPDATE ON device_ip 
FOR EACH ROW BEGIN
    SELECT RAISE(ROLLBACK, 'update on table "device_ip" violates foreign key constraint "fku_device_ip_id"')
    WHERE (SELECT id FROM device WHERE id = NEW.id_device) IS NULL;
END;
...
FOR EACH ROW BEGIN
    ...
    WHERE (SELECT id FROM domain WHERE id = NEW.device) IS NULL;
END;
